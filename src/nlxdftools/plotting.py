...
def format_load_params(df):
    if 'load_params' in df.attrs:
        params = '\n'.join([f'{k}={v}'
                            ...
                            if k != 'select_streams'])
        #title = (title + '\n' + textwrap.fill(params, 80))
    return params

def format_title(title, df):
    if (('load_params' in df.attrs
         and 'select_streams' in df.attrs['load_params'])):
        select_streams = ', '.join(
            [f'{list(select.keys())[0]}={list(select.values())[0]}'
             if isinstance(select, dict) else f'stream-id={select}'
...
def plot_sample_counts_df(df):
    n = df.shape[0]
    ax = df['sample_count'].unstack(level=1).plot.barh(
        colormap='tab20',
...
def plot_time_stamp_intervals_df(df, units='nanoseconds', showfliers=True):
    df = scale_seconds(df, units)
    n = df.index.levels[0].size
    n_devices = len(df.columns)
    cols = 2
    ...
    title = append_load_params(title, df)
    fig.suptitle(title)
    fig.tight_layout()
    return axes
...
def plot_first_time_stamps_df(df, units='seconds'):
    ...
    axes = axes.ravel()
    for (rec, data), i in zip(df.groupby(level=0, sort=False), range(0, n)):
        data.reset_index().plot.scatter(x='first_timestamp',
                                        y='stream_id',
                                        marker='|',
                                        ax=axes[i])
        axes[i].set_title(rec)
...
def plot_clock_offsets(data, normalise=False):
    ...
    maximum = 1
    if normalise:
        #maximum = list(data.values()).max()
        maximum = 50000
        minumim = 0
        print(maximum)
    fig, axes = plt.subplots(rows, cols, sharex=False, sharey=normalise,
                             figsize=(6 + cols, 4 + rows))
    axes = axes.ravel()
    ...
    title = append_load_params(title, list(data.values())[0])
    fig.suptitle(title)
